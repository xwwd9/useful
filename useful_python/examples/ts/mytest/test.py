...
def model_test():
    ...
    saver = tf.train.import_meta_graph(
        MODEL_SAVE_PATH + "crack_captcha.model-6200.meta")
...
# sess = tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for fileName in nameList:
            img_data, img_name = get_image_data_and_name(fileName,
                                                         TEST_IMAGE_PATH)
            predict = sess.run(predict_max_idx,
                               feed_dict={input_holder: [img_data],
            ...
            if np.array_equal(predictValue, rightValue):
                ...
                result = '错误'
            print('实际值：{}， 预测值：{}，测试结果：{}'.format(rightValue, predictValue,
                                                  result))
            print('\n')

        print('正确率：%.2f%%(%d/%d)' % (
        count * 100 / totalNumber, count, totalNumber))
